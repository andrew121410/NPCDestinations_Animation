...
public class FishingAssistant_1_10_R1 implements FishingAssistant {
    private double hookGravity = 0.125D;

    public FishingAssistant_1_10_R1(Double gravity) {
        hookGravity = gravity;
    }
        ...
        if (animationSettings.fishHook != null) {
            ...
            while (counter < 50) {
                ...
                for (byte y = -5; y <= 1; y++) {
                    ...
                    if (checkLocation.distanceSquared(npc.getEntity().getLocation()) > 8 && checkLocation.clone().add(0, -1, 0).getBlock().getType() == Material.STATIONARY_WATER && checkLocation.clone().getBlock()
