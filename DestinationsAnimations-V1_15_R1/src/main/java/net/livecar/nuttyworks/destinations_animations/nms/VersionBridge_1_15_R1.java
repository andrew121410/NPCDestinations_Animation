...
public class VersionBridge_1_15_R1 implements VersionBridge {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            switch (blockLocation.getBlock().getType()) {
                ...
                case BARREL:
                    ...
                    String bdString = blockLocation.getBlock().getBlockData().getAsString();
                    if (open)
                        bdString = bdString.replaceAll("open=false","open=true");
                    else
                        bdString = bdString.replaceAll("open=true","open=false");
                    blockLocation.getBlock().setBlockData(Bukkit.createBlockData(bdString));
    ...
    public void PlaySound(Location location, Animations_Location setting) {
        PlaySound(location, setting.sound, setting.soundCategory, setting.volume, setting.pitch);
    }
    
    public void PlaySound(Location location, Sound sound, Animations_Location.enSoundCategory soundCategory, float volume, float pitch) {
        for (Player plrEntity : Bukkit.getOnlinePlayers()) {
            if (plrEntity.getWorld() == location.getWorld()) {
                if (plrEntity.getLocation().distanceSquared(location) < 2116) {
                    ...
                    try {
                        sndCat = SoundCategory.valueOf(soundCategory.toString());
                    } catch (Exception err) {
                    }
                    
                    plrEntity.playSound(location, sound, sndCat, volume, pitch);
                }
            }
        }
    }
                        ...
                        if (compBlock.getLevel() >= compBlock.getMaximumLevel()) {
                            for (int emptySlot = 0; emptySlot < npcInventory.length; emptySlot++) {
                                ...
                                if (npcInventory[emptySlot] == null || npcInventory[emptySlot].getType() == Material.AIR) {
                                    PlaySound(composter, Sound.BLOCK_COMPOSTER_READY, Animations_Location.enSoundCategory.BLOCKS, 1, 16);
                            ...
                            PlaySound(composter, Sound.BLOCK_COMPOSTER_FILL_SUCCESS, Animations_Location.enSoundCategory.BLOCKS, 1, 16);
                            compBlock.setLevel(compBlock.getLevel() + 1);
                        }
                    } else {
                        PlaySound(composter, Sound.BLOCK_COMPOSTER_FILL, Animations_Location.enSoundCategory.BLOCKS, 0.3F, 0.8F);
                    }
